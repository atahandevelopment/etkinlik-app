...
const syncEvents = async () => {
    ...
    try {
        ...
        do {
            // Veriyi çek
            const { data } = await axiosInstance.get(`/events?take=${pageSize}&skip=${skip}`);
            ...
            if (eventsData && eventsData.length > 0) {
                ...
                if (eventsWithoutId.length > 0) {
                    ...
                    console.log(`${skip}'inci veriden sonraki ${pageSize} veri senkronizasyonu yapıldı.`);
                }
            }
...
const syncVenues = async () => {
    ...
    try {
        ...
        do {
            // Veriyi çek
            const { data } = await axiosInstance.get(`/venues?take=${pageSize}&skip=${skip}`);
        ...
        if (data && data.length > 0) {
            ...
            if (districtsWithId.length > 0) {
                for (const districtWithId of districtsWithId) {
                    const existingEvent = await Districts.findOne({ 'id': districtWithId.id });

                    if (!existingEvent) {
                        await Districts.create(districtWithId);
        ...
        if (data && data.length > 0) {
            ...
            if (categoriesWithId.length > 0) {
                for (const categoryWithId of categoriesWithId) {
                    const existingEvent = await Categories.findOne({ 'id': categoryWithId.id });
        ...
        if (data && data.length > 0) {
            ...
            if (formatsWithId.length > 0) {
                for (const formatWithId of formatsWithId) {
                    const existingEvent = await Categories.findOne({ 'id': formatWithId.id });
